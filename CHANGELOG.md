...
Added Apple Intelligence capability to Apple-silicon Mac definitions.
Made Built-for-iPad-on-macOS battery data report unavailable instead of a misleading 1% level.
Fixed MacBook Pro 2021 models being incorrectly identified with an intel chip.
Expanded EnvironmentsView into a tappable inline list with animated colored labels.
** App Store above **
Updated dependencies.
Updated CHANGELOG format.
Moved EnvironmentsView into Compatibility and shifted logic there.
Deprecated `Device.Environment` in favor of `Build.Environment`.
Deprecated `defaultFallback` in favor of `defaultUnknownSymbol`.
Deprecated CurrentDevice and Device environment wrappers in favor of Compatibility.Build checks and removed duplicated environment state from actual and mock devices.
Updated package and framework versions to match the current changelog entry.
Fixed Swift Playgrounds run sessions being reported as previews in Device Info.
Fixed IdentifyModelParsing support IDs so localized `EN-US/` prefixes are dropped from calculated IDs.
Made device diff text selectable without the disabled scroll wrapper flipping view modes or replacing formatted text with editable text fields.
Changed the Bridge/Device diff switch to a segmented picker so selecting text in diff content does not toggle the wrapper view.
Fixed lookup for Apple support page MacBook Air M5 names that include a trailing 2026 year, restoring color disambiguation.
Moved split Apple Watch GPS aluminum color lists into named watchXgps color sets and made cellular watch color sets extend those GPS sets with premium finishes.
Updated Apple Watch identify-page parsing.
Added determinate progress reporting for migration views.
Improved MacLookup color parsing by resolving colors against the matched local Mac name when source names include extra year or combined CPU wording.
Added additional DeviceKit bridges for migration from DeviceKit.
Fixed first-generation and Series 2 watch images to use aluminum for 38mm entries and ordinary stainless steel for 42mm entries.
Reduced fuzzy device lookup work by filtering unmistakable product-family hints to their matching idiom before calculating match scores.
Cached normalized and yearless official device names for the lifetime of the process and prepared normalized hint values once per lookup run.
Split grouped DeviceKit records when their identifiers resolve to multiple distinct local device definitions.
Added the iCloud key-value store identifier entitlement required by the sample app's Compatibility-backed cloud storage.
Added a deterministic SwiftPM test target guarded out of Swift Playgrounds manifests.
Configured Swift Package Index to build Device API documentation and documented local documentation and test workflows.
Replaced deprecated application-wide UIKit window lookup with scene-specific window enumeration for screenshot capture.
...
## v2.12.4 2026-05-11
Moved `String.defaultFallback` out of SwiftUI-only code so capability symbol fallbacks compile on Linux, WASM, Android, and other non-SwiftUI platforms.

## v2.12.3 2026-04-14
Fixed unnecessary PAT reference in README.  Addressed issue #7 (macs compatible with Tahoe incorrectly listed as not compatible).  Updated Compatibility & Color.
...
## v2.6.4 2025-01-15
Fixed missing package version update.  Updated resolved package versions. *PASSES ALL SWIFTPACKAGEINDEX TESTS*

## v2.6.3 2024-12-20
Set CURRENT_PROJECT_VERSION = ${MARKETING_VERSION} so that it will set the correct value when submitting to Mac App Store.  Fixed missing version update in Device.swift. *PASSES ALL SWIFTPACKAGEINDEX TESTS*
App Store Change Log:
Added MacBook Air 2024 & 2023 models.

## v2.6.2 2024-12-15
Added iMac "M4" models.  Renamed `macbookSilver` to `solidSilver`.  Fixed crashes by making sure official names were never identical and images are different and not reused. *PASSES ALL SWIFTPACKAGEINDEX TESTS*
App Store Change Log:
...
## v2.6.1 2024-11-26
Added new MacBook Pro M4 and Mac mini M4 models.  Updated dependent frameworks.

## v2.6.0 2024-10-25
Added new iPad mini devices.  Test Device with setting idle timer disabled on launch to make sure works as intended (updated instructions to indicate this needs to be done on view appear and NOT in the init).  Updated the idle timer toggle to reference the current state rather than a shadow variable.  Updated Apple Watch images to match the Identify your Apple Watch support page: https://support.apple.com/en-us/108056 *PASSES ALL SWIFTPACKAGEINDEX TESTS*

## v2.5.5 2024-10-18
Tried updating minimum Color framework to try to fix Linux support (Does!) *PASSES ALL SWIFTPACKAGEINDEX TESTS*

## v2.5.4 2024-10-14
Should fix Linux support by updating Compatibility framework. *PASSES ALL SWIFTPACKAGEINDEX TESTS - except Linux*

## v2.5.3 2024-10-13
Fixed Release build information by changing visibility of ActualHardwareDevice.  This addresses DTS Case-ID: 8753208. *PASSES ALL SWIFTPACKAGEINDEX TESTS - except Linux*

## v2.5.2 2024-10-13
Moved Xcode project back to original name to prevent opening crash issues.  Fixed missing project marketing version update!  Updated resolved package versions.

## v2.5.1 2024-10-08
Needed to rename app for Mac App Store. *PASSES ALL SWIFTPACKAGEINDEX TESTS*

## v2.5.0 (build 2) 2024-10-07
added 512 version of app icon. *PASSES ALL SWIFTPACKAGEINDEX TESTS*

## v2.5.0 2024-10-07
Tweaked to allow releasing app on the App Store.  Since must be built for DEBUG, tweaked check for Migration code.  Re-worked AttributeListView so can include in framework.  Added links to support documents.  Including attribute list in device details.  Fixed duplicate Mac14,9.  Fixed so Thunderbolt named devices definitely show thunderbolt capability.  Uploaded to App Store as "Device Information Tool"."

## v2.4.4 2024-09-22
Added identifiers to device detail view.  Fixed Apple Watch 10 identifiers. *PASSES ALL SWIFTPACKAGEINDEX TESTS*

## v2.4.3 2024-09-21
Fixed CPU issues with new iPhones and `cameraControl` button and added new Apple Watch stubs and colors.  Added black titanium to Apple Watch Ultra 2.  Fixed deinit task warning with Xcode 16 GM.  Added wrapping on colors to prevent squishing.  Updated colors and added colors for Apple Watches before Series 6 and re-named some colors to match Apple CSS.  Added iPhone colors before iPhone 13.  Updated Apple TV HD image.  Updated iPhone images to all use the identification page for consistency (https://support.apple.com/en-us/108044).

## v2.4.2 2024-09-11
Added new iPhones and `cameraControl` button.  Apple Watch updates not available yet.  Attempted to improve background tint to match better.

## v2.4.1 2024-08-13
Updated Color/Compatibility versions to address Swift 5.9 issues.  Previous version removed the Resources build when standardizing Package.swift!  Restored.  Explains why Bundle.module was not available since no resources included in the bundle. *PASSES ALL SWIFTPACKAGEINDEX TESTS*

## v2.4.0 2024-08-13
Standardized Package.swift, CHANGELOG.md, README.md, and LICENSE.txt files.  Standardized deployment targets.  Switched dependency from Compatibility to Color so we don't have to re-write color parsing code.  Added new HomePod mini Midnight color.  Added supported versions to indicate the preinstalled version and maximum version of iOS/macOS/etc supported by each device.  Have the max version be the version AFTER the version, so for late 2016 Touchbook, `launchOSVersion = "10.12.1"` and `unsupportedOSVersion = "13.0"` (max would be 12.x) - if this is `nil`, this is still currently supported.

## v2.3.4 2024-07-19
Added Identifiable to Battery protocol which I think works because ObservableObject requires AnyObject.  Added details for battery tests so that description can be seen and small screens can see all details.  Moved fontSize off of individual views in batteryView in preparation for extracting size entirely so can use sizes externally without having to specify font size (unfortunately, still can't due to not being able to specify relative sizes compared to base, but at least we cleaned up the code some).  Fixed so setting low power mode doesn't cause view to publish changes while rendering. *PASSES ALL SWIFTPACKAGEINDEX TESTS*

## v2.3.3 2024-07-17
Updated to make sure ObservableObject fallback is present from compatibility when needed for Linux compatibility.  Re-worked environment checks so they can be called as static functions on Device that don't need to be actor-isolated.  This also has the benefit of providing DeviceKit compatibility and thus we have un-deprecated those static functions.  Removed several unused functions and moved some to Compatibility.  Set minimum Compatibility version to 1.0.18.

## v2.3.2 2024-07-15
Updated compatibility to fix linux and watchOS support.  Updated icons to use new themeing.  Improved ScreenInfoView on visionOS (and removed completely from DeviceInfoView since really irrelevant on visionOS).

## v2.3.1 2024-07-14
Decided not to @_exported Compatibility here.  Removed duplicative EdgeInsets.zero.  Updated Compatibility inclusion.  Added compatibility version to text description and test app.  Migrated BytesView to Compatibility.

## v2.3.0 2024-07-11
Re-worked so that now depends on Compatibility rather than including duplicate compatibility code in the framework.  Increased legacy support back to iOS 11 and watchOS 4 to match DeviceKit (without SwiftUI or CurrentDevice features since CurrentDevice requires ObservableObject to be supported...perhaps create stateless wrappers?).  Fixed so that Swift Playgrounds works even when package supports an older iOS version.  If someone needs this, let us know otherwise we'll assume it's not important for legacy projects.

## v2.2.2 2024-07-04
Fixed issue where Linux build was failing and some additional concurrency warnings.

## v2.2.1 2024-07-04
Added Sendable conformance to IdiomType by making structs immutable and thus easily sendable.  Moved Migration out of Device framework so not included in client code.  Fixed some additional methods to be public that were not.  Added additional contrast to Device idiom icons.

## v2.2 2024-07-04
Removed CustomStringConvertible conformance from Device since we want to have CurrentDevice implement this (so we can have for both Mocks as well as ActualDevice) and we don't want confusion between the two implementations.  Fixed identifiability of mocks.  Added Version.swift and additional extensions on OperatingSystemVersion for better output and compatibility of version information on ALL devices.  Added hostSystemVersion and hostSystemName for macCatalyst and Designed for iPad and Linux systems.  Changed version returns from string to OperatingSystemVersion (or Version structs).  Added compatibility so that old uses should work without changing code (though if you store in typed variables that were previously strings, this may require code updates).  Removed `Device.description` and `Device.safeDescription` to prevent conflicts with `CurrentDevice.description` since CurrentDevice inherits from DeviceType.  Renamed to `safeOfficialName` for clarity and deprecated `safeDescription`.  Added macOS system name hard-coded component lookup for better system software info display.  Removed green background on visionOS.  Fixed `isDesignedForiPad` code on visionOS.  Fixed SomeCurrentDeviceView by making the device an @ObservedObject.

## v2.1.19 2024-07-03
Ensured CurrentDevice objects are @MainActor isolated so can be used in concurrent code.

## v2.1.18 2024-07-03
Converted several static variables from `var` to `let` for clarity.  Added availability check for renamed API from kIOMasterPortDefault to kIOMainPortDefault.
...
## v2.1.16 2024-06-03
Removed code for Swift 5.7 & 5.8 since conditional code in the Package.swift file doesn't seem to work.
...
## v2.1.14 2024-06-02
Fixed so that MockBattery and MockDevice are visible in Linux.
...
## v2.1.12 2024-06-01
Re-worked so MonitoredDeviceBattery is not required by Linux.

## v2.1.11 2024-06-01
Completely re-worked Storage view to be more compact and expand for more information.

## v2.1.10 2024-05-31
Forgot to update versions!  Also added legacy symbol for screen size.
...
## v2.1.7 2024-05-25
Added Thunderbolt capability.  Added new iPads.  Re-worked package for better compatibility with swiftpackageindex.com platforms.  Added checks for SwiftUI for compilation compatibilty.  Added support for swift 5.7.
...
## v2.1.3 2024-04-22
Moved Environments view in CurrentDeviceInfoView.

## v2.1.2 2024-04-21
Updating version numbers.  Updating Environments to be centered and visible in tests.  Updated Backport and other UI functions that should be version-restricted.  Tested Apple Watch version (missed in 2.1 testing).  Fixed issue with storage Ints being too small (so specified Int64).  Fixed updates for macCatalyst.

## v2.1.1 2024-04-21
Updated to enable showing environments in CurrentDeviceInfoView.
...
## v2.0.9 2024-04-11
Fixed issue with watchOS not being able to disable idle timer.

## v2.0.8 2024-04-10
Updated license copyright.  Added ability to disable screen dimming/locking (and ability to monitor battery state to disable idle timer automatically when plugged in).  Added battery.isPluggedIn variable.  Fixed so that a battery monitor is triggered regardless of battery level or battery state changes.
...
## v2.0.6 2024-04-06
Re-worked resources so that custom symbols are accessable outside module.  Fixed so macOS reports actual identifier (wasn't able to get identifier before).

## v2.0.5 2024-04-03
Fixed so works on macOS.  Fixed so works in Swift Playgrounds (had to bring minimum iOS level up to 15.2).  Renamed projects in Package.swift to fix so previews load consistently in Swift Playgrounds and Xcode previews.

## v2.0.4 2024-04-02
Renamed project from DeviceTest to Device.  Fixed so supported on tvOS 14.  Added crash detection to newest Apple Watch models.  Backed minimum iOS version to iOS 14.  Changed watchOS minimum to 6.  Updated minimum deployment targets in README.  Added fallbacks for some iOS 15+ only features.

## v2.0.3 2024-04-02
Made screen size resolution width and height public.  Removed HardwareListView from Hardware.swift to prevent issues with integrations.

## v2.0.2 2024-03-31
Fixed so can run test app on iOS 15/16.  Added custom symbols for nicer UI and to make sure icons work on older iOS devices.  Added NFC, ApplePay, Barometer, and Ringer Switch to iPhones where missing.  Improved the display of the test view.

## v2.0.1 2024-03-31
Fixed version warning with String.split() not available on iOS < 16.  Improved package definition with annotations.  Added public initializers for Device views.  Fixed compatibility and buildling for iOS < 16 in test views.  Updated Readme to highlight the new syntax.
...
## v1.1.2 2024-03-21
Fixed problem where rounded display corners was left in the vision definition.  Preparing for re-write of the capabilities engine.

## v1.1.1 2024-03-20
Fixed so readme icon works again due to moved assets to Resources folder.  Added isApplePencilCapable flag for iPads.  Removed legacy unused screen attributes (should add those back in under capabilities once we switch models in v2).

## v1.1.0 2024-03-20
Added symbols for device idiom icons and various build modes.  Fixed TestView Any to String conversion issue.  Added dynamicIsland configuration parameter.  Added in missing iPhone 14 Pro Max and updated several screen sizes and some wrong support IDs.  Added isMini flag for HomePods.  Added Mac forms. Created test for (Designed for iPad) mode vs native mode.  Added XCode test project so that we can build on visionOS and tvOS and watchOS and various mac targets.  visionOS (Designed for iPad) properly reports the new isDesignedForiPad flag when using a compatibility app.  Added scroll view so visible on watchOS or in iPhone landscape.  Added tvOS, visionOS, macOS, and watchOS icons.

## v1.0.11 2024-03-11
Fixed BatteryView to make sure it updates when DeviceBattery updates.  Fixed so macOS reports charging instead of full when charging.

## v1.0.10 2024-03-11
Updated documentation.  Changed phone test color to red since gray is not very obvious.  Re-worked BatteryView so that it can be scaled.

## v1.0.9 2024-03-11
Fixed so that BatteryView actually uses the initialization parameters (formerly, would not because they were marked @State).

## v1.0.8 2024-03-09
Updated Battery to be protocol instead of just a class so that it can be Mocked.  Added Mocks.  Added BatteryView for displaying battery indicator.

## v1.0.7 2024-03-09
Removed accidentally left in debugging code.

## v1.0.6 2024-03-08
Replaced checks for OSes with canImport(UIKit).  Added some code so battery level works on macOS Catalyst.  Updated readme to indicate macOS support.  Updated Description of Current Device.  Fixed so identifier works correctly in macCatalyst.

## v1.0.5 2024-03-08
Fixed so name that appears in Package List is Device not DeviceTestApp.

## v1.0.4 2024-03-08
Fixed so UIDevice is available on visionOS (UIKit wasn't being included on that platform.)

## v1.0.3 2024-03-08
Tested for compatibility.  Updated Readme.  Fixed so previews work.  Added Test code.

## v1.0.2 2024-03-05
Re-worked so Disk functions are available even before iOS 11 by moving availability checks into functions.

## v1.0.1 2024-02-28
Heath's additions and re-working code for IdiomType so that we can subclass and have device initializers while still being value types.

## v1.0.0 2024-02-16
Initial Project based off of DeviceKit but designed to be more maintainable and compatible.
...
## Pricing Analysis
To cover development expenses, $0.99 one-time purchase.
...
I'm getting a "'windows' was deprecated in Mac Catalyst 15.0: Use UlWindowScene.windows on a relevant window scene instead" warning in Swift Playgrounds.  Please figure out the proper way to take screenshots in Mac Catalyst 15+ and implement that code path leaving the existing method as a fallback for earlier OS versions if the proper way isn't supported all the way back.
